...
class TestRendering(common.TransactionCase):
    ...
    def _render(self, value, type_num=TYPE_NUM):
        style_props = self.style_obj.merge([self.style])
        return self.style_obj.render(self.lang, style_props, type_num, value)

    def _compare_and_render(
        self, value, base_value, type_num=TYPE_NUM, compare_method=CMP_PCT
    ):
        style_props = self.style_obj.merge([self.style])
        r = self.style_obj.compare_and_render(
            self.lang, style_props, type_num, compare_method, value, base_value
        )[:2]
        if r[0]:
